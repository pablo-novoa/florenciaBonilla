<?php
/*
Plugin Name: SingleStroke: Post Formats
...
// DISCLAIMER
// ===================================
// This plugin is a FORKED version of Crowd Favorite, Ltd "CF Post Formats" and Vafpress Posts Formats UI
// Credits goes to them http://crowdfavorite.com and http://vafpress.com
...
if (!defined('SSPF_VERSION')) {

define('SSPF_VERSION', '1.1.0');

function sspf_base_url() {
	return trailingslashit(apply_filters('sspf_base_url', plugins_url('', __FILE__)));
}

function sspf_admin_init() {
	$post_formats = get_theme_support('post-formats');
	if (!empty($post_formats[0]) && is_array($post_formats[0])) {
		if (in_array('link', $post_formats[0])) {
			add_action('save_post', 'sspf_format_link_save_post');
		}
		if (in_array('status', $post_formats[0])) {
			add_action('save_post', 'sspf_format_status_save_post', 10, 2);
		}
		if (in_array('quote', $post_formats[0])) {
			add_action('save_post', 'sspf_format_quote_save_post', 10, 2);
		}
		if (in_array('video', $post_formats[0])) {
			add_action('save_post', 'sspf_format_video_save_post');
		}
		if (in_array('audio', $post_formats[0])) {
			add_action('save_post', 'sspf_format_audio_save_post');
		}
		if (in_array('video', $post_formats[0])) {
			add_action('save_post', 'sspf_format_gallery_save_post');
		}
	}
}
add_action('admin_init', 'sspf_admin_init');

// we aren't really adding meta boxes here,
// but this gives us the info we need to get our stuff in.
function sspf_add_meta_boxes($post_type) {
	if (post_type_supports($post_type, 'post-formats') && current_theme_supports('post-formats')) {
		// assets
		wp_enqueue_script('sspf', sspf_base_url().'js/admin.js', array('jquery'), SSPF_VERSION, true);
		wp_enqueue_style('sspf', sspf_base_url().'css/admin.css', array(), SSPF_VERSION, 'screen');

		wp_localize_script(
			'sspf',
			'sspf_post_format',
			array(
				'loading'      => __('Loading...', 'sspf'),
				'wpspin_light' => admin_url('images/wpspin_light.gif'),
				'media_title'  => __('Pick Gallery Images', 'sspf'),
				'media_button' => __('Add Image(s)', 'sspf')
			)
		);

		add_action('edit_form_after_title', 'sspf_post_admin_setup');
	}
}
add_action('add_meta_boxes', 'sspf_add_meta_boxes');
 ...
 * @return void
 */
function sspf_post_admin_setup() {
	$post_formats = get_theme_support('post-formats');
	if (!empty($post_formats[0]) && is_array($post_formats[0])) {
		global $post;
...
function sspf_format_link_save_post($post_id) {
	if (!defined('XMLRPC_REQUEST') && isset($_POST['_format_link_url'])) {
		update_post_meta($post_id, '_format_link_url', $_POST['_format_link_url']);
	}
}
// action added in sspf_admin_init()

function sspf_format_auto_title_post($post_id, $post) {
	// get out early if a title is already set
	if (!empty($post->post_title)) {
		return;
	}

	remove_action('save_post', 'sspf_format_status_save_post', 10, 2);
	remove_action('save_post', 'sspf_format_quote_save_post', 10, 2);
	...
	$title = apply_filters('sspf_format_auto_title', $title, $post);
...
function sspf_format_status_save_post($post_id, $post) {
	if (has_post_format('status', $post)) {
		sspf_format_auto_title_post($post_id, $post);
	}
}
// action added in sspf_admin_init()

function sspf_format_quote_save_post($post_id, $post) {
...
function sspf_format_video_save_post($post_id) {
...
function sspf_format_audio_save_post($post_id) {
...
function sspf_format_gallery_save_post($post_id) {
...
add_action('wp_ajax_sspf_gallery_preview', 'sspf_gallery_preview');

function sspf_post_has_gallery($post_id = null) {
	if (empty($post_id)) {
		$post_id = get_the_ID();
	}
...
function sspf_pre_ping_post_links($post_links, $pung, $post_id = null) {
...
add_filter('pre_ping', 'sspf_pre_ping_post_links', 10, 3);

// For integration with Social plugin (strips {title} from broadcast format on status posts)
function sspf_social_broadcast_format($format, $post) {
...
add_filter('social_broadcast_format', 'sspf_social_broadcast_format', 10, 2);
